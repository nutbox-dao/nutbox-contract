...
async function deployTaxedERC20StakingFactoryContract(CommunityFactoryAddress) {
    let factory = await ethers.getContractFactory('TaxedERC20StakingFactory')
    let contract = await factory.deploy(CommunityFactoryAddress);
    await contract.deployed();
    // console.log("✓ ERC20StakingFactory contract deployed", contract.address);
    return contract;
...
async function deploy(owner) {
    ...
    result.TaxedERC20StakingFactory = await deployTaxedERC20StakingFactoryContract(result.CommunityFactory.address);
    ...
    await result.Committee.adminAddContract(result.TaxedERC20StakingFactory.address);
